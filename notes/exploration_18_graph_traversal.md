...
- [4. 순회(Traversal) — 모든 노드 방문하기](#4-순회traversal--모든-노드-방문하기)
  ...
  - [4.4 ★ 보충 — 왜 "방문 기록(visited)"이 필요한가? 트리 vs 그래프](#44--보충--왜-방문-기록visited이-필요한가-트리-vs-그래프-브로-질문)
...
### 4.4 ★ 보충 — 왜 "방문 기록(visited)"이 필요한가? 트리 vs 그래프 (브로 질문)

> 브로: *"그래프 순회에선 방문했던 노드를 기록할 수밖에 없는 것이니?
>         트리 순회에선 그런 거 안 했잖아?"*
> → ★ 핵심 차이. DFS/BFS 코드에 `if node not in visited:`가 있는 이유.
...
★ 트리의 핵심: **부모에서 자식으로만 이동** (화살표가 아래로만).
- A 방문 → B, C로 감 → B에서 다시 A로 갈 **방법 자체가 없음**
- 어차피 못 돌아가니 "방문했는지" 체크할 필요 없음

#### 그래프(Graph) — 방문 기록 필수 ★

```
   A ── B      ← 무방향이면 A→B, B→A 둘 다 가능!
   │          ← "돌아가는 길"이 있으면 무한 루프 위험
   C ── D
```

★ 핵심: 그래프는 **갈 수 있는 길이 여러 개** + **돌아가는 길도 있을 수 있음**.
- A → B 방문 → B에서 다시 A로 갈 수 있음 → 또 A 방문 → 또 B → **무한 루프!**
- 그래서 `if node not in visited:` 체크가 필수 — "이미 왔던 곳이면 안 가"

#### 요약 — 트리 vs 그래프

| | 트리 | 그래프 |
|---|---|---|
| 구조 | 부모→자식 단방향 | 양방향 또는 다중 경로 가능 |
| 돌아갈 수 있나 | ❌ (자식→부모 길 없음) | ★ 가능 (사이클) |
| 방문 기록 필요? | 불필요 | **필수** ★ |
| 안 하면? | 괜찮음 | 무한 루프 위험 |

#### ★★ DeZero(계산 그래프 = DAG)는 어디에 해당? — 트리와 그래프의 중간!

★★ 핵심: **DAG는 트리와 그래프의 중간**이야!
- DAG는 **방향 + 비순환** → 사이클 없음 (트리처럼 무한 루프 안 남)
- 근데 **분기/합류 가능** (그래프처럼 같은 노드로 여러 경로 도달 가능)
  - 예: `add(x, x)` → x가 두 경로에서 합류 → 같은 노드를 두 번 방문 가능

→ 그래서 DeZero 역전파(fill_grad)에선:
- **사이클은 없음** (DAG라서) → 무한 루프 위험 X
- 근데 **합류 노드 중복 방문 가능** → step14에서 gradient **누적**(if None)으로 해결한 게 바로 이거!

#### ★★★ step14 누적 = 방문 기록의 우아한 대체

step14에서 `if x.grad is None: 대입 else: 누적` 한 게 사실 **"방문 기록"의 역할**이었어!
- `x.grad is None` = "아직 방문 안 함" → 첫 번째 grad 대입
- `x.grad is not None` = "이미 방문함" → 두 번째 grad부터 누적

→ 즉 DeZero는 "visited 배열" 대신 **`grad` 필드 유무로 방문 여부를 간접 추적**하는 거야. 우아한 설계!

★ ★ 핵심 통찰: **step14 누적과 step15 그래프 순회는 같은 문제를 다른 각도에서 푼 것**.
- step14: "같은 노드 두 번 방문하면 grad 합산" (관점: 데이터 처리)
- step15: "복잡한 그래프에선 순서 보장 필요" (관점: 순회 순서)
- 둘 다 DAG의 "분기/합류" 속성에서 비롯됨.
