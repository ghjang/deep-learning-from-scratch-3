...
> **시점**: step16 완료 후 (2026-07-31). 섹션 9 회수 업데이트 (2026-08-11, step25 이후).
> **상태**: 옵션 I(이터레이터 추출) 회수 완료 ✅, 옵션 N(Node 도입) 보류 💡
> **트리거**: 브로 통찰 2종 —
>   1. "그래프 노드 정보를 좀 더 잘 코드상에 분리표현하면 어떨까" (Node 상위 클래스)
>   2. "'순회' 용어를 떠올리니 이터레이터 패턴이 떠올라" (순회 제너레이터 추출)
> **추적**: 이슈 21번(Node + manim, 옵션 N), 이슈 32번(옵션 I — 회수 완료)
...
### ★ 옵션 I (이터레이터 추출) 회수 완료 (2026-08-11, step25 이후 리팩터)

**step25에서 graphviz 시각화(`fold_dot_graph`) 구현 중 회수 시그널 도달** (섹션 6 후보 B 예측 적중).
이슈 32번으로 추적하다가 step25 완료 후 본격 리팩터 진행. 결과:

- `iter_reverse_topo` 제너레이터를 `rezero/v1/core.py`에 신설.
- `fill_grad`와 `fold_dot_graph`가 모두 이 제너레이터를 소비하도록 리팩터 (섹션 4의 초안 설계 구현).
- 상세: REZERO_CHANGES.md 항목 037 / 이슈 32번 close 코멘트.

**본 초안(섹션 4)에서 3가지 조정** (step16 시점 → step25 현행 v1):
1. None 가드 추가 (`start_var.creator is None` → 빈 순회)
2. visited 표시 시점 변경 (yield 직전 → append 시점, 중복 append 방지)
3. `f.inputs` None 가드 추가 (pyright)

**옵션 I 위치 결정**: `core.py` (graph.py 별도 모듈 아님).
- 순환 참조 회피 (graph.py로 가면 core.py ↔ graph.py 순환 발생 → 지연 import 필요).
- 현재 순회 알고리즘 1개뿐이라 YAGNI. 미래에 알고리즘 여러 개 생기면 그때 분리.
- 지연 import 한 줄이면 순환 참조 해결되긴 하지만, 회피할 수 있는 순환을 굳이 만들 이유 없음.

**옵션 N (Node 도입)은 여전히 보류** — 이슈 21번으로 추적.

### 도입 여부: **옵션 I 회수 완료, 옵션 N 보류 (이슈로 추적)**

옵션 I 회수 이유 (step25 시점):
1. 순회 소비자가 2개(fill_grad, fold_dot_graph)가 되어 추상화 이득 체감 ★ 핵심
2. 시각화 시점(step25) = 섹션 6 후보 B 예측 적중
3. 학습 서사 자연스러움 (step16 통찰 → step25 회수)

옵션 N 보류 이유:
1. Node 도입은 타입 계층 변경이라 범위 큼
2. 현재 Variable/Function 구조로 충분
3. manim 본격 도입(#21) 시점에 가치 재평가
