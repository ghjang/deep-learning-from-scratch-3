...
> **시점**: step02 직후 (Function 클래스의 추상 메서드 구현 방식에서 출발)
> **동기**: step02에서 `forward()`를 추상 메서드로 만들 때 `raise NotImplementedError()`를 썼다. "더 깔끔한 공식 방법(`abc.ABC`)이 있다"는 언급이 나와서, 둘의 차이와 선택 기준을 깊이 파봄.
