...
- ★ 브로 대혼란 — "2차 미분 시에도 최초 입력값이 유지된 채? grad엔 n차 미분값, 리턴값엔 평가값?" → **정확히 보고 있었다** (서술 4항목 전부 검증 통과). 정리가 필요했던 것 → **탐구 노트 33**으로 승격
- 핵심: 모든 미분은 "x=2라는 한 점에서의 평가" (f''(2)라는 숫자). x.data는 불변, x.grad는 "최근 fill_grad 대상의 미분값"으로 그때그때 충전. step36의 100은 y의 2차 미분이 아니라 새 함수 z의 1차 미분

### 🔗 관련 링크

- [Issue 42번 — step36 진행 추적](https://github.com/ghjang/deep-learning-from-scratch-3/issues/42)
- [탐구 노트 33 — Define-by-Run의 평가 시점](./notes/exploration_33_evaluation_timing.md) — 브로 혼란의 완전 정리 (이 step 산출의 총정리)
- [탐구 노트 30 — double backprop 이론](./notes/exploration_30_double_backprop.md) — "미분 결과도 함수다"의 이론
- 정답지: `steps/step36.py` (z = gx³+y)

### 📝 코드 / 수식 메모

- 검증: y=x², gx=2x → z=8x³+x² → z'(2) = 24·4+4 = **100** ✓ (dezero 완전 일치)
- z(2) = 4³+4 = 68 — "미분 식의 재평가 + 원래 출력 재사용"이 한 식에서 동시에
...
## Step 34 — [3고지] sin 함수 고차 미분

**Issue**: [#40](https://github.com/ghjang/deep-learning-from-scratch-3/issues/40)
**완료일**: 2026-08-24
**상태**: ✅

### 📖 요약 (한 줄)

sin을 3번 미분해 y/y'/y''/y''' 곡선을 그려 **미분 순환**(sin→cos→-sin→-cos)을 실증 — 능력 시연 스텝 (실무 사용 빈도와 무관, 탐구 노트 30 실무 좌표 참고).

### 💡 통찰 / 배운 점 (★ 학습 시작 전 브로-AI 대화로 도출)

- **브로가 발견한 쟁점 — "우리 전제와 충돌?"**: v2 전제 4 "스칼라 출력" vs 정답지의 x = `np.linspace(-7, 7, 200)` **벡터**. 문서 전제 위반은 맞으나, element-wise 함수는 야코비안이 **대각행렬**이라 VJP = 대각 원소 곱 = `df(x) * upstream` — 우리 derivative hook 공식과 동일해 **동작할 것으로 예측** (실험으로 검증)
- **★ 두 축의 구분 (브로 통찰 — "피스와이즈와 인자수 처리가 겹쳐 보인다")**: 직교하는 두 축이 같은 메커니즘(`*inputs` + ndarray 연산)에 녹아 있어 헷갈림 — ① **함수 인자축** (f(x0,x1) 다변수 함수 — step11~13에서 명시 구현, derivative 튜플) / ② **데이터축** (data가 스칼라냐 벡터냐 — 명시 구현 없음, NumPy에 위임). 책도 4고지 전까지 이 구분을 명시 안 함
- **"우연히 맞아떨어진다"의 정확한 성격**: 조건부 필연 — 흐르는 건 엄밀히 VJP(야코비안-벡터 곱)인데, element-wise의 야코비안이 대각이라 우리 공식과 정확히 일치. "스칼라 전제"는 보수적 문서화였지 코드 강제가 아니었음 → 34 step 생존의 비밀
- **★ "야코비안은 흐르지 않는다" (브로 질문에서 교정)**: 지금도 4고지 이후에도 **야코비안 행렬 자체는 결코 만들어지거나 흐르지 않음** — 흐르는 건 언제나 gradient (VJP의 결과). 지금(3고지)은 `df(x) ⊙ upstream` (대각만 압축 전송하는 희소 야코비안 관점 — 비대각 0성분은 계산 안 함), 4고지+는 backward 공식이 `gy·Bᵀ` 같은 **전치행렬 곱 = 진짜 VJP**를 직접 계산. "야코비안이 흐른다"는 autograd 유명 오해 — 언제나 흐르는 건 gradient뿐


### ❓ 질문 / 막힌 점

- ★ **브로의 두 버전 대조 실험이 v1 시절 Neg 버그를 잡음** (이번 step 최대 사건):
  벡터/루프 결과는 서로 일치하는데 y'''만 수학과 불일치 → 그래프 덤프 + 수치 해부 →
  `Neg.derivative = lambda x: -1.0 * x`가 **도함수(-1 상수)가 아니라 원 함수(-x)를 반환하는 버그** (v1 step22부터).
  은닉 경로: 테스트가 순전파만 검사 + `Neg.backward` 첫 호출이 고차 미분 3차부터라 발동 지연.
...
- 3자 대조 (A 벡터 / B 루프 / 수학 기대값) 전부 True — "벡터화는 편리함이지 수학적 필연이 아니다" + element-wise 원소 독립성 실증
- 그래프: `output/sin_higher_deriv.png` — 4곡선 90도씩 phase shift (미분 순환)
- 테스트 222개 (Neg 회귀 3개 신규) + pyright 0 errors


---

## Step 35 — [3고지] 고차 미분 계산 그래프

**Issue**: [#41](https://github.com/ghjang/deep-learning-from-scratch-3/issues/41)
**완료일**: 2026-08-24
**상태**: ✅

### 📖 요약 (한 줄)

tanh의 고차 미분 그래프를 시각화 — 도함수(tanh' = 1 - tanh²)가 **자기 자신을 참조**해 미분마다 그래프가 폭증하는 것을 눈으로 확인 (그래프 3형태 중 '폭증' 케이스).

### 💡 통찰 / 배운 점 (★ 학습 시작 전 브로-AI 대화로 도출)

- **브로 사전 관찰**: "도함수에 자기 자신이 재귀적으로 나온다" + "미분 반복 시 계산 그래프가 점점 커지는 예" — 둘 다 정확. tanh' = 1 - tanh²에서 오른쪽의 tanh가 바로 자기 참조
- **sin(순환)과 대비**: sin은 cos→-sin→-cos로 **다른 함수들 사이를 순환** (4주기, 그래프 크기 일정) / tanh은 **자기를 다시 참조**해 미분마다 Tanh 노드가 그래프에 추가 — 폭증
- **★ 후보 8번 회수의 순간**: 브로가 "머릿속으로 안 그려진다"던 2층 그래프를 실제 DOT으로 확인 — 같은 x 객체 공유(id 검증), gy 리프, Tanh 재귀 노드


### ❓ 질문 / 막힌 점

- (미스터리 — 해결) 재사용형 실험서 Tanh 노드 0개로 나온 건 내 카운트 버그 (`label="Tanh"`가 `label="TanhY"` 미스) — 실제로는 체인 완전 포함
- (문법 혼동 — 해소) `with using_config(...)`를 "True일 때만 진입"으로 착각 — with는 **설정 변경 구간**이지 진입 조건이 아님 (탐구 노트 23 참조). backward는 항상 실행, 바뀌는 건 실행 동안의 전역 설정값뿐

### 🔗 관련 링크

- [Issue 41번 — step35 진행 추적](https://github.com/ghjang/deep-learning-from-scratch-3/issues/41)
- [탐구 노트 32 — 미분식 해부](./notes/exploration_32_derivative_anatomy.md) — 이 step 산출의 총정리
- `notes/RESEARCH_QUEUE.md` 후보 8번 회수 이력 — 큐 시스템 첫 완주 (등록→검증→승격)
- 그래프 10장: `output/tanh_gx{N}.png` (재호출 1~5차) + `tanh_gx{N}_reuse.png` (재사용 1~5차)

### 📝 코드 / 수식 메모

- Tanh 수치: y' = 1−tanh² (0.78644773 @x=0.5), y'' = −2·tanh·(1−tanh²) (−0.72686198) — 8자리 일치
- 폭증 실측: 재호출형 Tanh 2→4→8→16→32 (2^n), Function 10→29→87→261→791 (≈3배) / 재사용형 Tanh **1개 고정**, Function 9→23→63→179
- 파일 크기도 폭증의 척도: gx5 2740 KB vs gx5_reuse 2330 KB — Tanh 수 차이는 극적인데 크기가 비슷한 건 **폭증의 주 범인이 mul/sub 구조 복제**라는 것 (Tanh 재호출은 부차)
- Config.reuse_output 사용법: `with using_config('reuse_output', True): fill_grad(...)` — ★ 블록이 fill_grad를 감싸야 함 (derivative는 역전파 중 호출)
- 단독 사용성: 재호출형만 `Tanh().derivative()`로 f'를 일급 객체로 획득 가능 (재사용형은 self.output 의존 — 순전파 없이 존재 불가)


---

## Step 36 — [3고지] 고차 미분 이외의 용도

**Issue**: [#42](https://github.com/ghjang/deep-learning-from-scratch-3/issues/42)
**완료일**: 2026-08-24
**상태**: ✅ ★ 3고지 (step25~36) 완료!

### 📖 요약 (한 줄)

살아있는 그래프의 재사용 — `z = gx³ + y`: 미분 식(gx)과 원래 출력(y)을 **새 계산의 재료**로 조립. "미분 결과도 계산의 일등 시민" (Define-by-Run 완성형).

### 💡 통찰 / 배운 점 (★ 학습 시작 전 브로-AI 대화로 도출)

- **브로 사전 파악**: "딱히 내용이 없는 가이드 스텝 — 이런 식으로도 사용된다는 시연"
- **고차 미분과의 차이**: 같은 걸 계속 미분(재미분)하는 게 아니라, 그래프를 **다른 함수의 빌딩블록으로 조립** — 3고지를 닫는 용도 확장 데모
- **실무 연결**: gradient penalty (WGAN-GP), gradient를 입력으로 받는 메타러닝 ("learning to learn") — 탐구 노트 30 실무 좌표의 2차 사용례들이 정확히 이 패턴
- **검산**: x=2 → y=x², gx=2x → z = (2x)³+x² = 8x³+x² → z'(2) = 24·4+4 = **100** ✓ (dezero 완전 일치)
- ★ 브로 혼란 4연타 → 탐구 노트 33 승격: ① 평가 시점 (한 점의 미분) ② z의 의미론 (패턴은 WGAN-GP·메타러닝 실무 핵심) ③ data=규약적 불변/grad=기록 슬롯 ④ 두 갈래 수렴 (96+4=100) + 자기 성찰 "새 그래프 재생성이 낯설었다" → step09 혼란 재림 연결 → 그래프의 생애 (일회용=학습 vs 지속=고차 미분)
