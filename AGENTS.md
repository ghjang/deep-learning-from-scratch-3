...
### `rezero/` 디렉터리 구조 (버전 폴더 전략 — step23 도입, step32에서 v2 브랜칭, 2026-09-04 v0 신설)

> ★ **step32에서 v2 탄생** (고차 미분 구현. grad ndarray→Variable은 API 호환성이
> 깨지는 대개편이라 v2로 분기 — 책의 core_simple/core 이분법과 평행).
> ★ **2026-09-04 v0 신설** — "자연수에 나중에 추가한 0" (이슈 43 작업 3, 브로 아이디어).
> v1에서 NumPy를 걷어낸 학습 실험실 — 두 축(그래프/데이터) 분리 체감.
...
→ rezero는 **각 고지별로 별도 폴더(v0/v1/v2/v3)** 로 스냅샷을 둠.
  - `rezero/v0/` — ★ **2026-09-04 탄생** (이슈 43 작업 3, 브로 아이디어).
    NumPy를 걷어낸 **순수 Python 학습 실험실** — "자연수에 나중에 추가한 0".
    Variable.data가 float 하나. 배치 계산 = 루프 직접 작성 → 데이터축이
    코드에 명시적으로 드러남 (노트 34 "배치 = 독립 스칼라 실험의 묶음"의 구현판).
    스코프는 v1과 동일 (1차 미분). 시각화는 v0 스코프 아님.
...
#### 🎯 버전별 포인트 — 감동 포인트 / 감상 포인트 (2026-09-04 정리)

| 버전 | 핵심 포인트 | 감동 / 감상 포인트 |
|---|---|---|
| **v0** (순수 Python) | data=float, 루프로 배치 표현 | **"배치 = 독립 스칼라 실험의 묶음"**(노트 34)이 코드로 드러남 — 루프가 데이터축을 보여줌. NumPy 없이도 미분 기계는 완전 동작 |
| **v1** (NumPy 도입) | data=ndarray, grad=ndarray | **벡터화의 마법** — 배치 3점이 fill_grad **1번 호출**로 (v0에선 루프 3번). 브로드캐스팅으로 스칼라×배열도 자동 |
| **v2** (고차 미분) | grad=Variable, create_graph | **그래프는 항상 자란다** — 1계 미분 그래프가 새 계산을 낳고, 그게 또 미분 가능. "미분의 미분"이 자연스럽게 됨 |

#### 📝 NumPy가 해주는 일 — v0 걷어내기에서 발견한 목록 (2026-09-04)

v0을 만들며 v1에서 무엇을 걷어냈는지 = **NumPy가 대신해주던 일의 목록**:

| v1에 있던 것 | v0에서의 운명 | NumPy의 역할 |
|---|---|---|
| `as_array(x)` | → `as_float(x)` | 스칼라→ndarray **승격** (0차원을 0-dim 배열로) |
| `VariableDataPropertyMixin` (shape/dtype/size/__len__) | **전체 삭제** | 배열 속성들의 **위임처** — float엔 없는 개념 |
| `np.ones_like(data)` | → `1.0` | 씨앗 생성을 **배열 크기에 맞춰** 자동 |
| `Variable(np.array([1.0, 2.0, 3.0]))` + `fill_grad` 1번 | 루프로 Variable 3개 + fill_grad 3번 | **벡터화** — 여러 점을 하나의 배열 연산으로 |
| `1 * upstream_grad` (스칼라×배열) | `1 * upstream_grad` (스칼라×스칼라) | **브로드캐스팅** — 자동 형상 확장 |
| `np.sin(x)` | `math.sin(x)` | **범용함수 (ufunc)** — 배열에 스칼라 함수를 자동 적용 |
| derivative 반환 타입 `ndarray \| float` 이중 | `float` 단일 | **타입 균일화** — 모든 계산이 배열이거나 스칼라 |

→ 소스를 다 볼 필요 없음 — 이 표가 v0↔v1 대응의 요약. 상세는 `rezero/v0/core.py`
헤더 docstring + `rezero/v0/tests/test_v0.py`의 `TestRemovedNumpyHelpers` 참조.
