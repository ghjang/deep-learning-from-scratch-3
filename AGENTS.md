...
**상태 이모지 범례** (`LEARNING_PROGRESS.md` 등 학습 문서 전반에서 사용):
...
> 위치는 **섹션 경로**로 표기 (줄 번호 참조는 문서 수정 시마다 어긋나서 폐기).

### 코드 작성 시 (자동 적용)
| 원칙 | 위치 | 한 줄 요약 |
|---|---|---|
| ★ 빈 줄 자동 적용 | 학습 철학과 스타일 > 코드 작성 | 논리 블록 사이 빈 줄. `coding_style.md` 항목 1 |
| ★ 매직메서드는 클래스 안에 정의 | 학습 철학과 스타일 > 코드 작성 | `ClassName.__method__ = func` (클래스 밖 대입) 금지. `coding_style.md` 항목 7 |
| ★ 코드 수정 후 재실행 검증 | step 진행 워크플로 > 스텝 랩업 > 절차 2단계 | 주석만 고쳤어도 재실행. "보고 성실성" 원칙 연동 |
| ★ 주석/docstring 코드 일치 점검 | step 진행 워크플로 > 스텝 랩업 > 절차 2단계 | 변수 개명/예외 타입/시그니처 변경 시 주석·docstring 동기화 |

### 문서/커뮤니케이션 작성 시 (자동 적용)
| 원칙 | 위치 | 한 줄 요약 |
|---|---|---|
| ★ 한국어 한자/외국어 혼용 주의 | 학습 철학과 스타일 > 노트 작성 | 한국어 텍스트엔 한국어만. 한자는 괄호 안 표기 |
| ★ `#N` 자동 링크 사전 검증 | Known Gotchas 섹션 10 | 이슈/코멘트 작성 전 단독 `#N`(N≥9) 검색. oreilly-japan referenced 이벤트 영구 손상 방지 |
| ★ 유니코드 마이너스(U+2212) 금지 | 문서 작성 원칙 (본문 서술) | 마이너스는 ASCII `-`만. U+2212는 VSCode에서 노란 박스(tofu)로 렌더링됨 (2026-08-24 브로 발견 — `·` `²` `∂` 등 다른 특수문자는 폰트 지원 OK, 유지) |
| ★ 마크다운 산문의 배열 리터럴은 백틱 | 문서 작성 원칙 (본문 서술) | 산문의 `[[...]]`는 링크로 오인 렌더링 — 배열 리터럴은 인라인 코드로 (2026-08-30 브로 발견) |

### step 시작/완료 시 (체크리스트)
| 원칙 | 위치 | 한 줄 요약 |
|---|---|---|
| ★ 정확한 책 제목 확인 | 학습 철학과 스타일 > 반복 실수 방지 | AI 부연설명 괄호 제거. 4곳(이슈/progress/notes/docstring) 일치화 |
| ★ 정답지 인접 step 비교 | 학습 철학과 스타일 > 반복 실수 방지 | stepN-1/N/N+1 세 파일 나란히 비교. 코드 배분 착각 방지 |
| ★ 코드 스타일 점검 | step 진행 워크플로 > 스텝 랩업 > 절차 2단계 | 빈 줄/PEP8/Pylance 경고 |
| ★ 분리/이관 작업 시 체크리스트 | step 진행 워크플로 > 스텝 랩업 > 절차 2단계 | 목차/링크/앵커 일치, 깨진 링크 확인 |
| ★ "1 step = 1 커밋" 정신 | step 진행 워크플로 > 스텝 랩업 > 커밋 단위 정책 | 의미 단위로 묶되 분리가 자연스러우면 분리 |
| ★ 랩업 시 `#N` 검증 의무 | step 진행 워크플로 > 스텝 랩업 > 절차 6단계 연동 | close 코멘트 작성 전 단독 `#N` 검색 |
| ★ step 코드 작성 시작 조건 | step 진행 워크플로 > step 전환 규칙 | 브로 "코드 짜줘" 전까지 코드 작성/랩업 안 함. 학습 준비+가이드만 먼저 |

### 메타 원칙 (어디서나 적용)
| 원칙 | 위치 | 한 줄 요약 |
|---|---|---|
| 보고 성실성 | step 진행 워크플로 > 스텝 랩업 > 재실행 검증 내부 | 생략한 단계 있으면 "생략했다" 명시. 추측을 사실처럼 보고 금지 |
| 즉시 수정 원칙 | step 진행 워크플로 > 즉시 수정 원칙 | 작업 원칙 개선 필요하면 랩업으로 미루지 말고 즉시 제안 |
| 결정 요청은 1개씩 | 학습 철학과 스타일 > 코드 작성 (협업 정리 방식) | 전체 목록 예고(OK) → 결정은 1개씩만. 폭풍 출력 금지 |
...
**★ 유니코드 마이너스(U+2212) 금지 (필수)** (2026-08-24 추가):

AI가 코드/노트에 수학 표기를 쓸 때 마이너스는 **ASCII `-`(U+002D)만** 사용할 것.
`−`(U+2212 MINUS SIGN)는 금지.

- **사례**: step33~35의 docstring/출력 문자열과 노트들에 `−` 사용 → 브로의 VSCode에서
  **노란 박스(tofu)**로 렌더링되어 가독성 저하 (브로 발견). 9개 파일 일괄 치환으로 정리.
- **원인**: 수학 문서 관례(숫자 폭과 맞는 마이너스)를 흉내냈으나 폰트 지원이 제각각.
- **참고**: `·`(가운뎃점), `²`(위첨자), `∂`(편미분) 등은 폰트 지원이 넓어 유지 (브로 확인).
  코드 **출력 문자열**에는 이들도 상황에 따라 피할 것 (터미널 의존).

**★ 마크다운 산문의 배열 리터럴은 백틱 (필수)** (2026-08-30 추가):

노트/문서의 **산문(본문 문장)에 배열 리터럴을 쓸 땐 반드시 인라인 코드(백틱)로 감쌀 것.**

- **사례**: 노트 37 §4에서 산문에 `x = [[10, 20, 30], [40, 50, 60]]`을 백틱 없이 썼더니
  마크다운 파서가 대괄호를 **링크 문법으로 오인**해 링크로 렌더링 (브로 발견).
  `x = `[[10, 20, 30], [40, 50, 60]]`` 형태로 수정하여 해결.
- **원인**: `[...]`는 마크다운 링크/참조 링크 문법과 표기가 겹침 — 이중 대괄호 `[[...]]`는
  위키링크로 오인되기도 함. 코드 펜스(블록) 내부는 안전, 문제는 산문.
- **원칙**: 배열/코드성 표기는 산문에서도 백틱 — "코드는 코드스팬" 국룰.

**★ 영문 병기는 정의 표(한 장 요약)에서 (필수)** (2026-09-04 추가):

노트에 **용어 대응 표(한 장 요약 등)가 있으면 영문 병기는 거기에만** — 표가 사실상
용어 사전이자 최초 등장이므로, 이후 본문은 순수 한국어로 쓴다.

- **사례**: 노트 44에서 한 장 요약 표를 비워두고 §2 본문 헤딩에 병기함 — 표가 병기의
  최적 위치임을 브로 지적. 표로 이동, 본문은 회수.
- **표가 없는 노트**: 본문 **첫 등장 시에만** 괄호 병기 (매번 X) — 기존 관례 유지.
- **효과**: 표 = 용어 사전(찾아보기), 본문 = 서술 — 계층 분리로 병기 중복 제거.
...
- **커밋/푸시는 명시적 승인 후에**: 6단계에서 브로가 승인하기 전엔 커밋/푸시하지 않는다 (최우선 규칙 준수). step 전환(7단계)은 커밋/푸시 완료 후에만.
...
- **브로가 명시적으로 "커밋해줘/푸시해줘" 선언** (최우선 규칙 준수)
