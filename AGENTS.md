...
#### 주의: 원 저자 레포(oreilly-japan)에 "참조 이벤트" 영구 남음

잘못된 `#N` 참조는 GitHub 시스템이 **oreilly-japan 원본 이슈 페이지에 referenced 이벤트로 기록**함.
- 이벤트는 actor:ghjang 으로 명시됨
- 커밋 hash와 함께 "이 커밋이 이슈를 참조함"이 영구 로그로 남음
- 확인 (2026-07-28): oreilly-japan 이슈 10번/11번에 ghjang referenced 이벤트 3건 박혀있음
...
| **그냥 둠** | ✅ 현실적 | 무해한 학습 레포 활동, 자동 로그라 굳이 제거 안 함 |

→ **이 사태의 교훈**: 잘못된 `#N`을 쓰면 **우리쪽 링크는 고칠 수 있지만, 원본 이슈 페이지의 referenced 이벤트는 영구 남는다**. 이게 `#N` 규칙(#10)을 엄격히 지켜야 하는 진짜 이유.
