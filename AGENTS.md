...
### 10. 커밋 메시지에서 `#N` 쓰면 fork 원본 이슈로 잘못 연결됨 (GitHub 자동 링크)

이 레포는 **fork** (`parent: WegraLee/deep-learning-from-scratch-3`, 그 상위 `oreilly-japan`)이다.
커밋 메시지나 이슈 본문에 `#N` 형태를 쓰면, GitHub는 **이슈 자동 링크**로 렌더링한다.
...
- 우리 레포엔 #10/#11 이슈가 없음 (최대 #6)
- GitHub가 상위 fork에서 #10/#11을 찾아 연결 → 클릭 시 오라일리 원본 일본어 레포로 이동
- → "이슈 본문에 일본어 링크가 왜 있지?" 혼란 발생

**해결 규칙**:
- ❌ **탐구 노트 번호에 `#N` 쓰지 말 것** — `탐구 #10`, `#11` 등 금지
- ✅ **대안 표기**:
  - `탐구 노트 10번` (숫자만)
  - `exploration_10` (파일명)
  - `탐구 #10` 대신 `탐구 10` (빈칸)
- 이슈 참조(`#1`~`#6`)는 우리 레포 이슈라 안전 — 혼동 주의

→ fork 레포에서 `#N` 자동 링크는 GitHub 기본 동작이라 레포 설정으로 끌 수 없음.
**커밋 메시지 작성 습관**으로만 방지 가능.

**참고**: GitHub 이슈 자동 링크는 `org/repo#N` 형태로 명시적 지정도 가능.
예: `ghjang/deep-learning-from-scratch-3#6` (명시적, 안전).
