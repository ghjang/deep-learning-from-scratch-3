...
- **진행 중**: 0
- **막힘**: 0
- **마지막 업데이트**: 2026-09-02 — 이슈 45 투어 진행 중 (abs/identity/sin 완료), 노트 42 출산, 서브그룹핑+라벨+벡터 렌더링 구현, 이슈 50(v3 분류)/51(duplicate_shared) 등록, Abs+Identity 함수 추가
- ★ **다음 작업** (2026-09-02 저녁 갱신 — 새 세션용 인수인계):
  **이슈 45 함수 종류별 투어 — Stop 3부터 계속 (스텝바이스텝 모드)**

  **오늘 완료** (전부 커밋·푸시됨, 273 passed / pyright 0):
  - 노트 36 §8 보강 (혼동 풀이 3종, None은 0이 아님) / 노트 42 출산 (n^k 위계, 이름 없는 3계, Schwarz 압축)
  - REZERO_CHANGES #043 + 이슈 50 (v3 함수 3분류 — step37 회수)
  - plot_multi_layer_graph 대개선: 층 내 서브그룹핑(중첩 cluster), show_labels 옵션(< forward > / < 1st backward > 서수+앵글), show_value 배열 대응(_format_data + array_show_value), Graphviz 버그 2종 픽스(라벨 3회 차단 label="", margin=16)
  - Abs(부호 참조형, 값 참조 도함수) + Identity(상수형) v2 추가 — DerivativeFn에 ndarray 허용
  - 투어 Stop 1 abs / 1.5 identity / 2 sin(4계+8계) — 관찰+이슈 45 코멘트 기록 완료
  - duplicate_shared 옵션 시도 → 철회 → **이슈 51로 재시도 등록** (브로 요구 정의 + 실패 지점 + 명확화 포인트 3개 기록)
  ...
  **내일 할 일** (우선순위 순):
  ...
  1. **투어 Stop 3 tanh — 예측부터** (브로 예측 3개 대기 중):
     ...
     - ③ 두 모드의 미분값 동일성
     - 관찰: 두 모드 나란히 PNG, 몇 계까지 렌더링 버티나
  2. **투어 Stop 4 x²+y²** — 다변수 (대각만 채워짐 → Hessian 절반, 이슈 48 직결)
  3. **투어 Stop 5 정리** — x·y/sin·cos 복습 묶음 → 3패턴(비대각만/대각만/전부) 확정 → 이슈 48 데이터
  4. **★ 투어 종료 시: 특이사항 모아 노트 정리** (브로 명시 요청 — 이슈 45 스톱 코멘트들 중 대발견 승격)
  5. **2계 노드 이름 개선** (이슈 45 close 조건) — x.grad 3개 중복 문제 (hxx/hxy가 x.grad로 똑같이 보임), ∂²z/∂x∂y 유니코드 표기 도입 여부 = 브로 결정
  6. **이슈 45 close** — 위 완료 후
  7. 이후: 이슈 47 MatMul (step41) / step37 v3 창설 시 #043·이슈 50 회수 / 이슈 51 duplicate_shared 재시도 (브로 명확화 3포인트부터: 간선 실선 확정, 고아 허용 여부, 레이아웃 통제 수준)

  **투어 운영 원칙** (오늘 확립):
  - 스톱마다: 브로 예측 먼저 → 관찰 → 채점 → 이슈 45 코멘트 기록
  - 한 스톱씩 천천히 (한 번에 다 만들지 않기)
  - 투어 산출물: output/tour*.png — 스크립트는 /tmp (휘발성, 재작성 필요하면 이슈 45 코멘트 참조)

  **오늘의 학습 포인트** (Stop 1~2 발견 — 상세는 이슈 45 코멘트):
  - abs/identity: 그래프 동형이지만 값의 역사 다름 (값 복사 vs 순수 상수), 2계 None (그래프 단절)
  - abs 0지점: 미분 불가인데 autodiff 조용히 0 (subgradient 관례) — 수치미분도 0이라 gradient check 사각지대 (PyTorch 등 공통)
  - sin: 값 4주기 / 재료 2주기 (부호가 Neg로 흡수) / 층별 새 노드 4개 고정 → 총 그래프 선형 성장 — 순환형의 그래프적 정의
  - sin 그래프의 "지방": 1 곱셈 Mul들 (autodiff 정직성 — symbolic이면 축약될 것들, torch.compile 영역)
