...
- ★ **다음 작업** (2026-08-31 심야 갱신 — 새 세션용 인수인계):
  ...
  **내일 할 일** (우선순위 순):
  ...
  3. **다변수 층의 서브그룹핑** (브로 아이디어, 2026-08-31 심야):
     - cluster_1 안에 입력 변수별 서브그룹 (Graphviz 중첩 cluster)
     - 예: [z, [gx, gy]] → cluster_1 안에 cluster_1a(gx의 그래프) + cluster_1b(gy의 그래프)
     - seed 등 공유 노드는 서브그룹 밖(층의 최상위)에 배치 — '공통 재료'임을 시각화
     - sin(x)·cos(y) Hessian 4출력 그래프에서 특히 유용 (Hessian 행별 서브그룹)

  4. **이슈 45 close 조건**: 1~3 완료 후 종결

  5. **이슈 48 Hessian 실험** — 오늘 발견한 "비대각만 구해진다"의 후속
     - x·y vs x²+y² vs sin(x)·cos(y)의 Hessian 구조 비교
     - "대각/비대각/전부" 패턴이 함수 유형과 어떻게 대응하는지

  6. 이후: 이슈 47 MatMul (step41 시점) / 작업 3(v0) or 5(연습문제)
