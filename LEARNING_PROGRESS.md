...
- **진행 중**: 0
- **막힘**: 0
- **마지막 업데이트**: 2026-09-03 저녁 — 이슈 52 클로즈(derive/plot_derivatives), 투어 Stop 3 tanh 완주(두 모드+부동소수점 g3≠), seed 구분 기능(seed_border — 농도안 폐기→테두리+last 라벨), 그래프 상속 구조 해부(흡수 법칙), 이슈 53(부동소수점)·54(상속 시각화) 등록
- ★ **다음 작업** (2026-09-03 저녁 갱신 — 이어서 작업용 인수인계):
  **이슈 45 함수 종류별 투어 — Stop 4부터 (예측 3개 대기 중)**

  **오늘(9/3) 완료** (로컬 커밋 4개 대기, 282 passed / pyright 0):
  - 이슈 52 완결: derive + plot_derivatives (mode: Literal['all','last']) — 스크립트 1줄화
  - 투어 Stop 3 tanh: 재호출(×3 지수) vs 재사용(Tanh 1개, 여전히 지수 — 곱셈 분기 공범 발견), **두 모드 g3/g4 값 미세 차이** (부동소수점 → 이슈 53)
  - last 모드: 층 박스 + < Nth backward > 라벨, _render_dot 헬퍼 추출
  - seed 구분 3연타: 농도안 폐기 → **seed_border** (테두리 황금각 팔레트, 1계 gold 고정, last 라벨 seed1/2/... — 2개 이상일 때만)
  - **그래프 상속 구조 해부** (★노트 승격 최상위 후보 — 이슈 54에 뼈대):
    - 전체/새/물려받은 것 3분할 (x²: g1 전체3/새3/물려0, g3 전체7/새5/물려2)
    - 흡수 법칙: k계는 "derivative가 참조한 변수→태생 함수" 사슬만 흡수 (g1∩g3=0, g2∩g3=2, id 동일=같은 객체 재참조)
    - all 층 함수 수 = 새 것만 / last = 전체 (5 vs 7 미스터리 해결)
    - 씨앗 수 = backprop 호출 횟수 (다변수 무관 — 출력 쪽 dy/dy)

  **내일(9/4) 할 일** (우선순위 — ★주말까지 3고지 정리 목표):

  1. **투어 Stop 4 x²+y²** — 예측 3개 대기 중:
     - ① gx의 그래프에 y가 있을까 (참조 법칙 적용)
     - ② backprop(gx)의 x.grad/y.grad = 값 vs None (대각만 채워짐?)
     - ③ x·y(비대각)/x²+y²(?)/sin·cos(전부) 3패턴을 가르는 규칙
  2. **투어 Stop 5 정리** — 3패턴 확정 → 이슈 48 데이터 완성
  3. **★ 투어 종료 정리 — 특이사항 노트로** (브로 요청):
     - 이슈 54 내용 = 노트 43 뼈대 ("그래프 상속의 구조: 흡수 법칙")
     - 그 외 대발견: sin 지방 노드, 0지점 함정, 부호참조/상수형 동형, tanh 곱셈 분기, 부동소수점 g3≠
  4. **2계 노드 이름 개선** (이슈 45 close 조건) — x.grad 중복 문제, 빠른 결정만
  5. **이슈 45 close** + LEARNING_NOTES 투어 요약
  6. **(주말) step37 시작 검토 = 4고지 진입** — 브로 "고고" 선언 필요. v3 창설 + 이슈 50(함수 3분류) 회수

  **로컬 커밋 4개 푸시 대기**: a771158(가비지) 1f074dd(이슈52) 8cb68d4(seed_border) + 본 progress 커밋
  **산출물**: output/ 비움 — helper_x2_border_{all,last,order1}.png만 있음 (필요시 재생성 스크립트 /tmp/tour3_*.py)
