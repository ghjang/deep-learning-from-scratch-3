"""
rezero.tests.test_deconv2d — [5고지] Deconv2d(전치합성곱) forward/backward
====================================================

이 파일은 rezero 구현체의 Deconv2d(전치합성곱) forward/backward 을(를) 검증하는 자리입니다.

패턴 (원본 tests/test_deconv2d.py 참고):
...
class TestDeconv2d(unittest.TestCase):
    """TODO: Deconv2d(전치합성곱) forward/backward 검증 케이스를 여기에 구현하세요.

    예시 (원본 tests/test_deconv2d.py 스타일):
