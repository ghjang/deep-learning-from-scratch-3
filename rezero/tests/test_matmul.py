"""
rezero.tests.test_matmul — [3고지] 행렬 곱(matmul)의 forward/backward
====================================================

이 파일은 rezero 구현체의 행렬 곱(matmul)의 forward/backward 을(를) 검증하는 자리입니다.

패턴 (원본 tests/test_matmul.py 참고):
...
class TestMatmul(unittest.TestCase):
    """TODO: 행렬 곱(matmul)의 forward/backward 검증 케이스를 여기에 구현하세요.

    예시 (원본 tests/test_matmul.py 스타일):
