"""
rezero.tests.test_weight_decay — [4고지] 가중치 감쇠 (L2 정규화)
====================================================

이 파일은 rezero 구현체의 가중치 감쇠 (L2 정규화) 을(를) 검증하는 자리입니다.

패턴 (원본 tests/test_weight_decay.py 참고):
...
참고 자료:
  - 정답지 테스트: tests/test_weight_decay.py
  - 헬퍼 함수: rezero/utils.py 의 gradient_check, array_allclose (직접 구현 필요)
  - 프레임워크 자료: dezero/utils.py

실행:
  uv run python -m unittest rezero.tests.test_weight_decay
...
class TestWeightDecay(unittest.TestCase):
    """TODO: 가중치 감쇠 (L2 정규화) 검증 케이스를 여기에 구현하세요.

    예시 (원본 tests/test_weight_decay.py 스타일):
