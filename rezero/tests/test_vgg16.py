"""
rezero.tests.test_vgg16 — [5고지] VGG16 모델 forward (가중치 로드 포함)
====================================================

이 파일은 rezero 구현체의 VGG16 모델 forward (가중치 로드 포함) 을(를) 검증하는 자리입니다.

패턴 (원본 tests/test_vgg16.py 참고):
...
class TestVGG16(unittest.TestCase):
    """TODO: VGG16 모델 forward (가중치 로드 포함) 검증 케이스를 여기에 구현하세요.

    예시 (원본 tests/test_vgg16.py 스타일):
