"""
rezero.tests.test_broadcast — [3고지] 브로드캐스트 (broadcast_to)의 형상 처리
====================================================

이 파일은 rezero 구현체의 브로드캐스트 (broadcast_to)의 형상 처리 을(를) 검증하는 자리입니다.

패턴 (원본 tests/test_broadcast.py 참고):
...
class TestBroadcast(unittest.TestCase):
    """TODO: 브로드캐스트 (broadcast_to)의 형상 처리 검증 케이스를 여기에 구현하세요.

    예시 (원본 tests/test_broadcast.py 스타일):
