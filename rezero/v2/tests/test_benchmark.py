...
from rezero.v2 import Variable, fill_grad, numerical_diff  # noqa: E402
...
class TestSphere:
    ...
    def test_gradient_at_1_1(self):
        ...
        assert y.grad.data == np.array(2.0)
    ...
    def test_minimum_at_origin(self):
        ...
        assert x.grad is not None
        assert x.grad.data == np.array(0.0)
        assert y.grad is not None
        assert y.grad.data == np.array(0.0)
...
class TestMatyas:
    ...
    def test_gradient_at_1_1(self):
        ...
        assert np.allclose(x.grad.data, np.array(0.04))
        assert y.grad is not None and y.grad.data is not None
        assert np.allclose(y.grad.data, np.array(0.04))   # 대칭
    ...
    def test_gradient_symmetric(self):
        ...
        assert x.grad.data is not None and y.grad.data is not None
        assert np.allclose(x.grad.data, y.grad.data)
...
class TestGoldstein:
    ...
    def test_gradient_at_1_1(self):
        ...
        fill_grad(z)
        assert x.grad is not None
        assert x.grad.data == np.array(-5376.0)
        assert y.grad is not None
        assert y.grad.data == np.array(8064.0)
