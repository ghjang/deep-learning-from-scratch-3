...
class TestBasicBackward:
    def test_square_backward(self):
        ...
        y = square(x)
        fill_grad(y)
        assert x.grad == np.array(6.0)   # 2*3
...
class TestMultivariableBackward:
    def test_add_backward(self):
        ...
        from rezero.v1 import add
        a = Variable(np.array(2.0))
        b = Variable(np.array(3.0))
        y = add(a, b)
        fill_grad(y)
        assert a.grad == np.array(1.0)
        assert b.grad == np.array(1.0)

    def test_mul_backward(self):
        """y = a * b, dy/da = b, dy/db = a."""
        from rezero.v1 import mul
        a = Variable(np.array(3.0))
        b = Variable(np.array(2.0))
        y = mul(a, b)
        fill_grad(y)
        assert a.grad == np.array(2.0)   # b
        assert b.grad == np.array(3.0)   # a
...
class TestNoGrad:
    ...
    def test_no_grad_restore_then_backprop_works(self):
        ...
        y = square(x)
        fill_grad(y)
        assert x.grad == np.array(6.0)   # 2x 정상
    ...
    def test_using_config_general_form(self):
        ...
        from rezero.v1 import Config, using_config
        assert Config.enable_backprop is True
...
class TestRetainGrad:
    def test_retain_grad_false(self):
        ...
        assert x0.grad == np.array(2.0)   # ★ 정확값 (step18 기대)
        assert x1.grad == np.array(1.0)   # ★ 정확값
    ...
    def test_retain_grad_true(self):
        ...
        # ★ 수치 기대값 (step18 데모)
        assert y.grad == np.array(1.0)    # 최종 출력
        assert t.grad == np.array(1.0)    # 중간
        assert x0.grad == np.array(2.0)   # 최종 입력 (두 경로)
        assert x1.grad == np.array(1.0)   # 최종 입력
...
class TestWeakrefOutput:
    ...
    def test_fill_grad_raises_if_output_collected(self):
        ...
        fill_grad(y)
        assert x.grad == np.array(4.0)   # 2*2
